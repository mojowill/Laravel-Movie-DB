<x-main>
    <div class="border-b border-gray-800 movie-info">
        <div class="container flex flex-col px-4 py-16 mx-auto md:flex-row">
            <div class="flex-none">
                <img src="{{ $person['profile_path'] }}" alt="profile image" class="w-76">
                <ul class="flex items-center mt-4">

                    @if ($social['facebook'])
                        <li>
                            <a href="{{ $social['facebook'] }}" title="Facebook">
                                <svg class="w-6 text-gray-400 fill-current hover:text-white" viewBox="0 0 448 512">
                                    <path
                    ...
                    @if ($social['twitter'])
                        <li class="ml-6">
                            <a href="{{ $social['twitter'] }}" title="Twitter">
                                <svg class="w-6 text-gray-400 fill-current hover:text-white" viewBox="0 0 512 512">
                                    <path
                    ...
                    @if ($person['homepage'])
                        ...
                        <li class="ml-6">
                            <a href="{{ $person['homepage'] }}" title="Website">
                                <svg class="w-6 text-gray-400 fill-current hover:text-white" viewBox="0 0 496 512">
                                    <path
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-sm text-gray-400">
                    <svg class="w-4 text-gray-400 fill-current hover:text-white" viewBox="0 0 448 512">
                        <path
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-5">
                    @foreach ($knownForMovies as $movie)
                        <div class="mt-4">
                            <a href="{{ $movie['media_type'] == 'movie' ? route('movies.show', $movie['id']) : route('shows.show', $movie['id']) }}"><img src="{{ $movie['poster_path'] }}" alt="poster" class="transition duration-150 ease-in-out hover:opacity-75"></a>
                            <a href="{{ $movie['media_type'] == 'movie' ? route('movies.show', $movie['id']) : route('shows.show', $movie['id']) }}" class="block mt-1 text-sm leading-normal text-gray-400 hover:text-white">{{ $movie['title'] }}</a>
                        </div>
